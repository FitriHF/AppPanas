package id.ac.amikom.apppanas.data.local
...
import id.ac.amikom.apppanas.util.getObject
import id.ac.amikom.apppanas.util.putObject
...
class AuthPref(val context: Context) {
    ...
    suspend fun login(email: String, password: String): ActionState<AuthUser> {
        val user = authUser
        return if (user == null) {
            ActionState(message = "Anda balum daftar", isSuccess = false)
        } else if (email.isBlank() || password.isBlank()) {
            ActionState(message = "Email or pass tidak boleh kosong", isSuccess = false)
        } else if (user.email == email && user.password == password) {
            isLogin = true
            ActionState(authUser, message = "Berhasil Login")
        } else {
            ActionState(message = "Email or pass SALAH", isSuccess = false)
        }
    }
    ...
    suspend fun logout(): ActionState<Boolean> {
        isLogin = false
        return ActionState(true, message = "Berhasil Logout")
    }
}